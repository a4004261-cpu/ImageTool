...
export async function POST(req: NextRequest) {
  try {
    ...
    if (prompt.length > 400) return NextResponse.json({ ok:false, error:'Prompt is too long.' }, { status:400 });
    if (!ratios[ratio]) return NextResponse.json({ ok:false, error:'Invalid ratio.' }, { status:400 });
    ...
    return NextResponse.json({ ok:true, provider:'mock', imageDataUrl, width, height }, { headers:{'Cache-Control':'no-store'} });
  } catch {
